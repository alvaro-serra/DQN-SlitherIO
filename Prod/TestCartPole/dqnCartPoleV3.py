...
TARGET_NETWORK_UP_FREQ = 5000 # frequency with which the target network is updated. Parameter C. #test 100
    ...
    for i_episode in range(starting_episode,10000000):
        ...
        while not done_n:
            ...
            if t>OBSERVATION and t%UPDATE_FREQ == 0 and train:
                ...
                feed_train = {image: inputs, actions_ref: targets, keep_prob: KEEP_PROB, actionhist: inpActHist}
                real_error,_, current_loss = sess.run([error, train_step, loss_value], feed_dict = feed_train)
                mean_batch_loss[t%200] = current_loss
                print(clipped_error(real_error))
