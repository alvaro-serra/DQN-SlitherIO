...
TARGET_NETWORK_UP_FREQ_EP = 5 #2 # frequency with which the target network is updated. Parameter C. #test 100
...
with tf.Session(graph = ConNet) as sess:
    ...
    if next(os.walk("./train"))[1] == []:
        ...
        D,epsilon,starting_episode,reward100 = pickle.load(fload)
        fload.close()
        t = lastmodel
    ...
    for i_episode in range(starting_episode,10000000):
        ...
        actionidx = 0
        action_n = [idx2act(actionidx) for ob in observation_n]
        ...
        while not done_n[0]:
            ...
            if done_n[0]:# Punish hard when failing
                ...
                #print('FINALLY OVER!!!')
                #print(state_p1)
                D.append((state_p0,actionidx,reward_n[0],state_p1,done_n[0],acthist,acthistp1))
            ...
            if not done_n[0]:
                reward_n[0] = 1./(1.+np.exp(-reward_n[0])) #clip rewards [0,1]
    ...
    #Checked        ## IN EPISODE: If we have filled enough the buffer we perform replay memory
            ...
            if t % MODEL_SAVER == 0 and t>OBSERVATION:
                ...
                pickle.dump([D,epsilon,i_episode+1,reward100],fsave)
                fsave.close()
